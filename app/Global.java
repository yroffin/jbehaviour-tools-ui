...
public class Global extends GlobalSettings {
	...
	public void initializeData(Application app) throws Exception {
		...
		if (!localStoryApp.initialize(Play.application().configuration())) { throw new Exception(
	...
	public void initialize(Application app) {
		...
		String namespaceAware = app.configuration().getString(
				SPRING_NAMESPACE_AWARE);
		String addPlayProperties = app.configuration().getString(
				SPRING_ADD_PLAY_PROPERTIES);
		...
		if (url == null) {
			Logger.info("Loading default application context: application-context.ml");
			url = app.classloader().getResource("application-context.xml");
		}
		if (url != null) {
			InputStream is = null;
			try {
				Logger.info("Starting Spring application context from "
						+ url.toExternalForm());
				applicationContext = new GenericApplicationContext();
				Spring.applicationContext = applicationContext;
				applicationContext.setClassLoader(Play.application()
						.classloader());
				...
				if (!"false".equalsIgnoreCase(addPlayProperties)) {
					...
					for (String key : keys) {
						try {
							...
							// Some config items are complex, so we'll just skip
							// those for now.
						}
					}
				...
				try {
					...
					throw new RuntimeException(
							"Unable to instantiate Spring application context",
							ex);
				} finally {
	...
	void finalyze(Application app) {
		...
		try {
			...
			Throwable ex = e.getCause();
			throw new RuntimeException(
					"Unable to instantiate Spring application context", ex);
		} finally {
